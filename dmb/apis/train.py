...
import torch
from mmcv.runner import Hook, DistSamplerSeedHook, obj_from_dict
from dmb.utils.runner import Runner
...
def batch_processor(model, data, train_mode, epoch=-1, cal_cov=False, cov_list=None):
    if train_mode:
        if cal_cov:
            assert cov_list is None
            model.eval()
            if epoch >= 10:
                with torch.no_grad():
                    cov_list = model.module.raw_arr(data)
            else:
                cov_list = None
            model.train()
            return cov_list
        ...
        model.eval()
    _, losses = model(data, epoch, cov_list)
